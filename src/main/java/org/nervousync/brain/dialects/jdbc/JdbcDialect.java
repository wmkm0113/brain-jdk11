...
public abstract class JdbcDialect extends BaseDialect {
	...
	/**
	 * <span class="en-US">Drop view command</span>
	 * <span class="zh-CN">删除视图命令</span>
	 */
	...
	/**
	 * <span class="en-US">Select distinct command</span>
	 * <span class="zh-CN">唯一不同值命令</span>
	 */
	private static final String DISTINCT_COMMAND = "DISTINCT";
	/**
	 ...
	 */
	public final Properties properties(final TrustStore trustStore, final Authentication authentication) {
		Properties properties = new Properties();
		this.trustStoreProperties(properties, trustStore);
		if (authentication instanceof UserAuthentication) {
			...
			this.authProperties(properties, authentication);
		}
	 ...
	 */
	public String createDatabase(final String shardingName, final String databaseParameters) {
		return CREATE_DATABASE + this.nameCase(shardingName) + databaseParameters;
	}
	 ...
	 * @return <span class="en-US">Data view name</span>
	 * <span class="zh-CN">视图名称</span>
	 */
	public final String viewName(@Nonnull final String tableName) {
		if (StringUtils.isEmpty(tableName)) {
			return Globals.DEFAULT_VALUE_STRING;
		}
		return "[" + this.nameCase(tableName) + "_View]";
	}
	 ...
	 * <span class="zh-CN">生成的SQL命令</span>
	 */
	public final String createShardingView(@Nonnull String tableName, final List<String> shardingTables) {
	...
	/**
	 * <h3 class="en-US">Drop view command</h3>
	 * <h3 class="zh-CN">删除分片数据表的视图</h3>
	 *
	...
	/**
	 * <h3 class="en-US">Generate index name</h3>
	 * <h3 class="zh-CN">生成索引名称</h3>
	 *
	 * @param indexName <span class="en-US">Index name</span>
	 *                  <span class="zh-CN">索引名称</span>
	 * @param tableName <span class="en-US">Database table name</span>
	 *                  <span class="zh-CN">数据表名</span>
	 * @return <span class="en-US">Generated index name</span>
	 * <span class="zh-CN">生成的索引名称</span>
	 */
	public final String indexName(final String indexName, final String tableName) {
	...
	public final List<String> createIndexCommand(@Nonnull final TableDefine tableDefine, final String shardingTable) {
		...
		for (IndexDefine indexDefine : tableDefine.getIndexDefines()) {
			...
			stringBuilder.append(this.nameCase(this.indexName(indexDefine.getIndexName(), tableName)))
	...
	public final String dropIndexCommand(final String indexName, final String tableName) {
		return DROP_INDEX + this.nameCase(this.indexName(indexName, tableName)) + DEFAULT_COMMAND_ON + this.nameCase(tableName);
	}
	 ...
	 * <h3 class="zh-CN">处理连接需要使用的身份认证信息</h3>
	 *
	 * @param properties     <span class="en-US">Connect properties instance object</span>
	 *                       <span class="zh-CN">连接属性值</span>
	 * @param authentication <span class="en-US">Authentication information</span>
	...
	/**
	 * <h3 class="en-US">Generate alias name mapping table</h3>
	 * <h3 class="zh-CN">生成别名映射表</h3>
	 *
	...
	private void aliasNames(@Nonnull final Map<String, String> aliasMap, @Nonnull final QueryFrom queryFrom,
		...
		if (queryJoinList != null && !queryJoinList.isEmpty()) {
			for (QueryJoin queryJoin : queryJoinList) {
				if (queryJoin instanceof SubQueryJoin) {
					...
					if (!aliasMap.containsKey(tableQueryJoin.getJoinTable())) {
						String aliasName = StringUtils.isEmpty(queryJoin.getAliasName())
								? "t_" + aliasMap.size()
								: queryJoin.getAliasName();
						if (!aliasMap.containsValue(aliasName)) {
							aliasMap.put(tableQueryJoin.getJoinTable(), aliasName);
						}
					}
				}
			}
	...
	/**
	 * <h3 class="en-US">Handle the mapping relationship between sub-query data columns and data types</h3>
	 * <h3 class="zh-CN">处理子查询数据列和数据类型的映射关系</h3>
	 *
	...
	/**
	 * <h3 class="en-US">Generate SQL commands to query items</h3>
	 * <h3 class="zh-CN">生成查询数据列命令</h3>
	 *
	...
	private String itemCommand(@Nonnull final Map<String, String> aliasMap, @Nonnull final List<QueryItem> itemList,
					...
					.ifPresent(item -> {
						if (queryItem instanceof ColumnItem && ((ColumnItem) queryItem).isDistinct()) {
							if (distinctBuilder.length() > 0) {
								distinctBuilder.append(BrainCommons.DEFAULT_SPLIT_CHARACTER);
							}
							distinctBuilder.append(item);
						} else {
		...
		if (distinctBuilder.length() > 0) {
			itemBuilder.insert(Globals.INITIALIZE_INT_VALUE, BrainCommons.DEFAULT_SPLIT_CHARACTER)
					.insert(Globals.INITIALIZE_INT_VALUE, BrainCommons.BRACKETS_END)
					.insert(Globals.INITIALIZE_INT_VALUE, distinctBuilder)
					.insert(Globals.INITIALIZE_INT_VALUE, BrainCommons.BRACKETS_BEGIN)
					.insert(Globals.INITIALIZE_INT_VALUE, DISTINCT_COMMAND);
		}
		...
		for (JoinInfo joinInfo : queryJoin.getJoinInfos()) {
			...
			switch (joinInfo.getConditionCode()) {
				case LESS:
					...
					break;
				case LESS_EQUAL:
					columnBuilder.append(BrainCommons.OPERATOR_LESS_EQUAL);
					break;
				case EQUAL:
					columnBuilder.append(BrainCommons.OPERATOR_EQUAL);
					break;
				case NOT_EQUAL:
					columnBuilder.append(BrainCommons.OPERATOR_NOT_EQUAL);
					break;
				case GREATER:
					columnBuilder.append(BrainCommons.OPERATOR_GREATER);
					break;
				case GREATER_EQUAL:
	...
	private String subQuery(final Map<String, String> aliasMap, @Nonnull final AbstractQuery queryData,
		...
		if (queryData.getGroupByList() != null && !queryData.getGroupByList().isEmpty()) {
			...
			for (GroupBy groupBy : queryData.getGroupByList()) {
				...
				groupByClause.append(this.columnName(aliasMap, groupBy.getTableName(), groupBy.getColumnName()));
			}
	 ...
	 * @param aliasMap  <span class="en-US">Data table alias mapping table</span>
	 *                  <span class="zh-CN">数据表别名映射表</span>
	 * @param queryItem <span class="en-US">Query item define information</span>
	...
	private String queryItem(final Map<String, String> aliasMap, final QueryItem queryItem,
		...
		switch (queryItem.getItemType()) {
			...
			case COLUMN:
				...
				break;
			case CONSTANT:
				ConstantItem constantItem = queryItem.unwrap(ConstantItem.class);
				if (calculateParam) {
				...
				break;
			case CALCULATE:
				CalculateItem calculateItem = queryItem.unwrap(CalculateItem.class);
				...
				for (QueryItem paramItem : calculateItem.getCalculateItems()) {
					if (sqlBuilder.length() > 0) {
				...
				break;
			default:
				throw new MultilingualSQLException(0x00DB00000016L, queryItem.getItemType());
		}
		return sqlBuilder.toString();
	}
